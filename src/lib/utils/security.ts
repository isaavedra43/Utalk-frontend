...
class SecurityManager {
    ...
    sanitizeContent(content: string): string {
        ...
        allElements.forEach(element => {
            const allowedAttrs = Array.from(element.attributes).filter(attr =>
     ...
     */
    async validateFiles(files: File[]): Promise<{ valid: boolean; error?: string; validFiles: File[] }> {
...
export const validateMessageByBytes = (content: string) => securityManager.validateMessageByBytes(content);
