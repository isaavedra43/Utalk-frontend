...
class MonitoringSystem {
  ...
  trackSocketEvent(
    ...
    const recentSocketErrors = this.socketMetrics.filter(
      m => !m.success && new Date(m.timestamp).getTime() > Date.now() - 60000 // Último minuto
    );
   ...
   */
  trackConversationsLoad(startTime: number, success: boolean, count: number, error?: string): void {
    this.trackPerformance('load_conversations', startTime, success, error, { count });
  ...
  trackMessagesLoad(
    ...
    this.trackPerformance('load_messages', startTime, success, error, { conversationId, count });
  ...
  trackMessageSend(
    ...
    this.trackPerformance('send_message', startTime, success, error, {
      conversationId,
      messageType,
      hasAttachments
    });
  ...
  trackFileUpload(
    ...
    this.trackPerformance('upload_file', startTime, success, error, { filename, size });
   ...
   */
  trackValidation(type: string, success: boolean, input: string, error?: string): void {
   ...
   */
  trackRateLimit(endpoint: string, remaining: number, resetTime: string): void {
  ...
  private isCriticalError(error: string): boolean {
    ...
    return criticalErrors.some(critical => error.toUpperCase().includes(critical));
  }
  ...
  cleanupOldMetrics(): void {
    ...
    this.errorMetrics = this.errorMetrics.filter(m => new Date(m.timestamp).getTime() > oneHourAgo);
  ...
  getMetricsSummary(): Record<string, unknown> {
    ...
    return {
      ...
      socketSuccessRate:
        totalSocketEvents > 0
          ? ((successfulSocketEvents / totalSocketEvents) * 100).toFixed(2) + '%'
          : '0%',
...
export const trackMessageSend = (
  ...
  monitoringSystem.trackMessageSend(
    startTime,
    success,
    conversationId,
    messageType,
...
export const trackValidation = (type: string, success: boolean, input: string, error?: string) =>
  monitoringSystem.trackValidation(type, success, input, error);

export const trackRateLimit = (endpoint: string, remaining: number, resetTime: string) =>
