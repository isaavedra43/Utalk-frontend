...
import type { Message, MessageFilters, MessagesState } from '$lib/types';
  ...
  return {
    ...
    loadMessages: async (conversationId: string, filters: MessageFilters = {}) => {
      ...
      try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const sortedMessages = (response as any).data.data.messages.sort(
          (a: unknown, b: unknown) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    loadMoreMessages: async () => {
      ...
      if (!currentState.hasMore || currentState.loading || !currentState.currentConversationId)
        return;
      ...
      try {
        ...
        const response = await api.get<unknown>(
          `/conversations/${encodeURIComponent(currentState.currentConversationId)}/messages?${params.toString()}`
        );
      ...
      try {
        ...
        await executeUpdate(() => {
          update(state => ({
            ...
            messages: state.messages.map(m =>
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              (m.metadata as any)?.localOnly ? { ...m, status: 'failed' } : m
            ),
            loading: false,
