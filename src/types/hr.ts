...
// ==========================================
// NUEVAS INTERFACES PARA SISTEMA AVANZADO DE NÓMINA
// ==========================================
...
export interface AdvancedPayrollConfig extends PayrollConfig {
  // Configuración de impuestos opcionales
  taxSettings: TaxSettingsConfig;
  
  // Integración con extras
  extrasIntegration: {
    autoApplyExtras: boolean;      // Aplicar automáticamente movimientos de extras
    requireApproval: boolean;      // Requerir aprobación antes de aplicar
    trackDuplicates: boolean;      // Rastrear duplicados
    markAsApplied: boolean;        // Marcar movimientos como aplicados
  };
}
...
export interface PayrollMovement {
  id: string;
  employeeId: string;
  type: 'bonus' | 'overtime' | 'deduction' | 'loan' | 'advance' | 'commission';
  amount: number;
  description: string;
  date: string;
  ...
  isDuplicate: boolean;            // Marcado como posible duplicado
  originalMovementId?: string;     // ID del movimiento original si es duplicado
  preventDuplication: boolean;     // Prevenir duplicación
  duplicateCheckFields: string[];  // Campos para verificar duplicados
  applicationSource: 'manual' | 'automatic' | 'import'; // Fuente de aplicación
...
export interface AdvancedPayrollPeriod extends PayrollPeriod {
  // Desglose avanzado
  extrasApplied: Array<{
    movementId: string;
    type: string;
...
export interface DuplicateAnalysis {
  ...
  recommendations: string[];
  autoResolutionSuggestions: Array<{
    action: 'merge' | 'keep_original' | 'keep_duplicate' | 'manual_review';
    reason: string;
    confidence: number;
  }>;
}